...
def generate_file_sentiment(basicout_path, cutoff, file_sentiment_path):
    ...
    with open(file_sentiment_path, 'a') as csv_out:
        ...
        if os.stat(file_sentiment_path).st_size == 0:
            writer.writeheader()
        for dct in dct_lst:
            writer.writerow(dct)
...
def main(input_path, output_folder, config_file, cutoff, gold_file=""):
    pos_mark = "positive"
    neg_mark = "negative"
    ...
    if os.path.isfile(input_path):  # 1 single file
        ...
        cmd = "python3 SO_Calc.py -i \"" + input_path + "\" -bo \"" + basicout_path + "\" -ro \"" + richout_path + "\" -c \"" + config_file + "\""
        os.system(cmd)
        ...
        for f_name in os.listdir(input_path):
            ...
            if not os.path.isfile(file_path):
                continue
            cmd = "python3 SO_Calc.py -i \"" + file_path + "\" -bo \"" + basicout_path + "\" -ro \"" + richout_path + "\" -c \"" + config_file + "\""
            os.system(cmd)
